...
test_that("makeCross",{
  SP = SimParamEpidemic$new(founderPop=founderPop) # ada
  SP$nThreads = 1L
  SP$addTraitA(nQtlPerChr=1) # ada
  SP$setTrackPed(TRUE)
  ...
  expect_all_equal(unname(meanG(pop1)),0) #ada
...
test_that("makeCross2",{
  ...
  expect_equal(SP$lastId,12L)
  expect_all_equal(unname(meanG(pop1)),0)
  expect_equal(unname(SP$pedigree[-(1:2),1L]),rep(1L,10L))
  expect_equal(unname(SP$pedigree[-(1:2),2L]),rep(2L,10L))
  #Match by id
  crossPlan = matrix(as.character(crossPlan),ncol=2)
